...
11. Put only that handoff in the new task's prompt. Preserve its ID, title, complete `descriptionMarkdown`, `timingText`, `requestedDestinationText`, and `referenceContentEntityIds`. Tell the new task that it exclusively owns completing this handoff and must call `drain_handoff` with the handoff ID and `processingTarget` only after the work is actually complete.
12. The coordinator must not execute handoff work, edit handoff target files, wait for or supervise created tasks, or call `drain_handoff`. A successfully created and registered task means the handoff was routed, not processed or drained.
